...
namespace NT.DeviceDriver.Beta
{
    public class USYS2000COM : IDeviceDriver
    {
        ...
        #region 字段、属性与构造方法
        private readonly string _driverName = "Beta_BLM-C+";
        /// <summary>
        /// 配置文件
        /// </summary>
        ...
        /// 配置文件
        /// </summary>
        private string _defaultConfigFile = "Resources\\DeviceDriver\\Default\\Beta_BLM-C+.json";
        /// <summary>
        /// 配置文件
        /// </summary>
        ...
        private void Connect(ConnectConfig connectConfig)
        {
            ...
            try
            {
                if (_client == null)
                {
                    ...
                    _client.SocketBufferSize = Convert.ToUInt32(connectConfig.ReadBuffSize);
                    _client.KeepAliveTime = Convert.ToUInt32(connectConfig.Timeout);
                    RegisterEvent(_client);
                }
                var result = _client.Connect(connectConfig.Host, Convert.ToUInt16(connectConfig.Port));
                if (!result)
                {
        ...
        private void ReadData(byte[] buff)
        {
            try
            {
                ...
                if (address != null && !string.IsNullOrWhiteSpace(recv))
                {
                    ...
                    int index = startIndex + address.Length;
                    if (startIndex != -1 && recv.Length > index)
                    {
                        Dictionary<string, object> dataMap = new Dictionary<string, object>();
                        Dictionary<string, string> dataTypeMap = new Dictionary<string, string>();

                        string dataStr = recv.Substring(index).Trim();
            ...
            {
                string message = string.Format("设备数据采集失败：[IP：{0}，原因：{1}]", _deviceConfig.ConnectConfig.Host, ex.Message);
                OnError?.Invoke(new Exception(message), 1, _connectId);
            }
        }
        /// <summary>
