...
namespace NT.DeviceDriver.Beta
{
    ...
    /// 通讯协议的类库，多项式码0xA001
    /// </summary>
    public class BetaCom : SerialDeviceBase<ReverseWordTransform>
    {
        #region Constructor
        ...
        {
            OperateResult<byte[]> read = ReadBase(Encoding.ASCII.GetBytes(SendChar));
            if (!read.IsSuccess) return OperateResult.CreateFailedResult<short>(read);
            return OperateResult.CreateSuccessResult(Convert.ToInt16(Encoding.ASCII.GetString(read.Content)));
        }
        /// <summary>
        /// 读取short
        /// </summary>
        /// <returns>带有成功标志的字节信息</returns>
        public OperateResult<short> ReadShort(string SendChar, params string[] removeChars)
        {
            OperateResult<byte[]> read = ReadBase(Encoding.ASCII.GetBytes(SendChar));
            if (!read.IsSuccess)
            {
            ...
            {
                string str = Encoding.ASCII.GetString(read.Content);
                ...
                return OperateResult.CreateSuccessResult(Convert.ToInt16(str));
            }
        }
        ...
        public OperateResult<float> ReadFloats(string SendChar, ushort length)
        {
            ...
            string trimStr = Encoding.ASCII.GetString(read.Content).Trim().TrimStart('\r', '\n').TrimEnd('\r', '\n').Trim();
        ...
        /// </summary>
        /// <returns>带有成功标志的字节信息</returns>
        public OperateResult<float> ReadFloats(string SendChar, ushort length, params string[] removeChars)
        {
            OperateResult<byte[]> read = ReadBase(Encoding.ASCII.GetBytes(SendChar));
            if (!read.IsSuccess) return OperateResult.CreateFailedResult<float>(read);
            string amount = string.Empty;
            ...
            foreach (var removeChar in removeChars)
            {
                if (!string.IsNullOrWhiteSpace(removeChar))
                {
                    trimStr = trimStr.Replace(removeChar, "");
                }
            }
            if (IsNumeric(trimStr))
                amount = Convert.ToDecimal(trimStr).ToString(string.Format("F{0}", length));
        ...
        public bool IsNumeric(string str) //接收一个string类型的参数,保存到str里
        {
            ...
            {
                if ((c < 48 || c > 57) && c != 46 && c != 32)                          //判断是否为数字
                {
