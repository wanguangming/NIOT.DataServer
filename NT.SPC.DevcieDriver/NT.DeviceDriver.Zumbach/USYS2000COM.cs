...
{
    public class USYS2000COM : IDeviceDriver
    {
        ...
        #region 字段、属性与构造方法
        private readonly string _driverName = "Zumbach_USYS2000COM";
        /// <summary>
        /// 配置文件
        /// </summary>
        ...
        /// 配置文件
        /// </summary>
        private string _defaultConfigFile = "Resources\\DeviceDriver\\Default\\USYS2000COM.json";
        /// <summary>
        /// 配置文件
        /// </summary>
            ...
            while (_colloctMark)
            {
                ...
                try
                {
                    foreach (var address in _deviceConfig.AddressConfigs)
                    {
                        ...
                        if (!isValid)
                        {
                            string message = string.Format("设备访问失败：[串口：{0}，错误信息：{1}]",
                                _deviceConfig.ConnectConfig.PortName, errorMessage);
        ...
        private bool ReadDataFromAddress(AddressConfig address, out object value, out string errorMessage)
        {
            ...
            this._serialPort.Write(sendBuff, 0, sendBuff.Length);
            string dataStr = string.Empty;
            try
            {
                int length = _deviceConfig.ConnectConfig.ReadBuffSize;
            ...
            {
                string message = string.Format("设备连接失败：[串口：{0}，原因：{1}]", _deviceConfig.ConnectConfig.PortName, ex.Message);
                OnError?.Invoke(new Exception(message), 1, _connectId);
                _isConnected = false;
                return false;
            }
