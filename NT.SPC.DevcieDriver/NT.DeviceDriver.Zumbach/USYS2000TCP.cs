...
namespace NT.DeviceDriver.Zumbach
{
    public class USYS2000TCP : IDeviceDriver
    {
        ...
        #region 字段、属性与构造方法
        private readonly string _driverName = "Zumbach_USYS2000TCP";
        /// <summary>
        /// 配置文件
        /// </summary>
        ...
        /// 配置文件
        /// </summary>
        private string _defaultConfigFile = "Resources\\DeviceDriver\\Default\\USYS2000TCP.json";
        /// <summary>
        /// 配置文件
        /// </summary>
        ...
        private void Connect(ConnectConfig connectConfig)
        {
            ...
            if (this._tcpClient == null)
            {
                ...
                _tcpClient.ReceiveBufferSize = connectConfig.BufferSize;
                _tcpClient.ReceiveTimeout = connectConfig.Timeout;
            }
            try
            {
                _tcpClient.Connect(connectConfig.Host, connectConfig.Port);
            }
            catch (Exception ex)
            {
                string message = string.Format("设备连接失败：[主机：{0}:{1}，原因：{2}]", connectConfig.Host, connectConfig.Port, ex.Message);
            ...
            {
                string message = string.Format("设备连接失败：[主机：{0}:{1}]", connectConfig.Host, connectConfig.Port);
                OnError?.Invoke(new Exception(message), 2, _connectId);
                _isConnected = false;
            }
            else
        ...
        private void CollectData()
        {
            while (_colloctMark)
            {
                ...
                {
                    string message = string.Format("设备数据采集失败：[主机：{0}:{1}，错误信息：{2}]",
        ...
        private bool ReadDataFromAddress(AddressConfig address, out object value, out string errorMessage)
        {
            ...
            byte[] sendBuff = Encoding.ASCII.GetBytes(command);
            this._tcpClient.Client.Send(sendBuff, sendBuff.Length, 0);
            string dataStr = string.Empty;
            try
            {
                int length = _tcpClient.Client.ReceiveBufferSize;
                byte[] buff = new byte[length];
                this._tcpClient.Client.Receive(buff);
                ...
                if (!string.IsNullOrWhiteSpace(recv))
                {
                    ...
                    recv = recv.Substring(startIndex + address.Address.Length);
                    int endIndex = recv.IndexOf("\r\n");
                    if (endIndex == -1)
                    {
                        endIndex = recv.IndexOf("\r");
                        if (endIndex == -1)
                        {
                            endIndex = recv.IndexOf("\n");
                        }
                    }
                    if (endIndex > 0)
                    {
                        recv = recv.Substring(0, endIndex);
                    }
                    dataStr = recv.Trim();
                }
            }
            ...
            {
                string message = string.Format("设备连接失败：[主机：{0}:{1}，原因：{2}]", _deviceConfig.ConnectConfig.Host, _deviceConfig.ConnectConfig.Port, ex.Message);
                OnError?.Invoke(new Exception(message), 1, _connectId);
                _isConnected = false;
                return false;
            }
            if (!string.IsNullOrWhiteSpace(dataStr))
            {
                try
                {
                    ...
                    {
                        case "bool":
                            value = Convert.ToBoolean(dataStr);
                            break;
                        case "ushort":
                            value = Convert.ToUInt16(dataStr);
                            break;
                        case "short":
                            value = Convert.ToInt16(dataStr);
                            break;
                        case "uint":
                            value = Convert.ToUInt32(dataStr);
                            break;
                        case "int":
                            value = Convert.ToInt32(dataStr);
                            break;
                        case "float":
                            value = Convert.ToSingle(dataStr);
                            break;
                        case "double":
                            value = Convert.ToDouble(dataStr);
                            break;
                        case "string":
                            value = dataStr;
                            break;
