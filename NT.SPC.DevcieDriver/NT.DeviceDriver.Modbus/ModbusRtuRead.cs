...
{
    /// <summary>
    /// 设备自主发送Modbus-RTU格式数据（无需采集命令）
    /// </summary>
    public class ModbusRtuRead : IDeviceDriver
    {
        ...
        #region 字段、属性与构造方法
        private readonly string _driverName = "Modbus Rtu Read";
        /// <summary>
        /// 配置文件
        /// </summary>
        ...
        /// 配置文件
        /// </summary>
        private string _defaultConfigFile = "Resources\\DeviceDriver\\Default\\Modbus-RTU-Read.json";
        /// <summary>
        /// 配置文件
        /// </summary>
        ...
        private void SerialPort_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            Thread.Sleep(_deviceConfig.ConnectConfig.Timeout);
        ...
        private bool ReadDataFromBytes(AddressConfig address, byte[] buff, out object value, out string errorMessage)
        {
            ...
            if (resIndex == 0)
            {
                byte length = buff[resIndex + 10];
                byte[] data = buff.Skip(resIndex + 11).Take(length).ToArray();
