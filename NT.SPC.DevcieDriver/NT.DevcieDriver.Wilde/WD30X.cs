...
namespace NT.DevcieDriver.Wilde
{
    public class WD30X : IDeviceDriver
    {
        ...
        #region 字段、属性与构造方法
        private readonly string _driverName = "Wilde-WD30X";
        /// <summary>
        /// 配置文件
        /// </summary>
        ...
        /// 配置文件
        /// </summary>
        private string _defaultConfigFile = "Resources\\DeviceDriver\\Default\\Wilde-WD30X.json";
        /// <summary>
        /// 配置文件
        /// </summary>
        ...
        public class ConnectConfig
        {
            ...
            /// 采集周期
            /// </summary>
            public int ColloctCycle { get; set; } = 1000;
            /// <summary>
            /// 通讯超时
            /// </summary>
        ...
        public class AddressConfig
        {
            ...
            /// </summary>
            public string Convert { get; set; } = "21";
            /// <summary>
            /// 数据倍率
            /// </summary>
        ...
        private void CollectData()
        {
            while (_colloctMark)
            {
                ...
                try
                {
                    ...
                    {
                        var isValid = ReadDataFromAddress(address, out float odx, out float ody, out string errorMessage);
                        dataMap.Add("ODX", odx);
                        dataTypeMap.Add("ODX", address.DataType);
                        dataMap.Add("ODY", ody);
                        dataTypeMap.Add("ODY", address.DataType);
                        dataMap.Add("OD", (odx + ody) / 2);
                        dataTypeMap.Add("OD", address.DataType);
                        float oval = 0;
                        if (odx != 0 && ody != 0)
                        {
                            if (odx > ody)
                        ...
                        dataMap.Add("Oval", Math.Abs(oval));
                        dataTypeMap.Add("Oval", address.DataType);
                        if (!isValid)
                        {
                            hasError = true;
                ...
                Thread.Sleep(_deviceConfig.ConnectConfig.ColloctCycle);
            }
        }
        /// <summary>
        /// 从地址读取数据
        ...
        /// <param name="odx">X向线径</param>
        /// <param name="ody">Y向线径</param>
        /// <param name="errorMessage">错误信息</param>
        /// <returns>数据是否有效</returns>
        private bool ReadDataFromAddress(AddressConfig address, out float odx, out float ody, out string errorMessage)
        {
            bool isValid = false;
            odx = 0;
            ody = 0;
            ...
            OperateResult<byte[]> result = _modbus.Read(key, address.Length);
            if (result.IsSuccess)
            {
                ushort valuex = ByteArrayToUint16(result.Content[0], result.Content[1]);
                ushort valuey = ByteArrayToUint16(result.Content[2], result.Content[3]);
                odx = ConvertValue(valuex, address);
                ody = ConvertValue(valuey, address);
                isValid = true;
            }
            return isValid;
        }
        ...
        /// <returns></returns>
        private ushort ByteArrayToUint16(byte high, byte low)
        {
            return Convert.ToUInt16(high * 0x100 + low);
        }
        /// <summary>
        /// 转换数据的值
        /// </summary>
        /// <returns></returns>
        private float ConvertValue(object value, AddressConfig address)
        {
            float temp = Convert.ToSingle(value);
            if (address.Convert.Equals("21"))
            {
